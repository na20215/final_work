...
image_dir = 'D:\\dataset\\grey\\train\\image\\door\\'
label_dir = 'D:\\dataset\\grey\\train\\label\\'
classes = ['door_heat_losing', 'door_not_heat_losing']
batch_size = 32
img_width = 160
img_height = 160
num_channel = 3
...
class_weight = {0: 134/206, 1: 72/206}
images_file = glob.glob(os.path.join(image_dir, '*.jpg'))
images = len(images_file)
dataset = dataset_building.create_dataset_for_door(image_dir, label_dir, classes, img_width, img_height, num_channel)
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.MaxPooling2D((2,2)),
    tf.keras.layers.Conv2D(128, (5, 5), activation='relu', padding='same', kernel_initializer='glorot_uniform'),
    tf.keras.layers.MaxPooling2D((2,2)),
...
model.save('D:\\models\\door_judgment_model.h5')
